export const translations = {
  en: {
    home: "Home",
    garden: "Garden",
    archive: "Archive",
    tags: "Tags",
    about: "About",
    search: "Search...",
    search_placeholder: "Search...",
    no_results: "No results found.",
    latest_writing: "Latest Writing",
    curated_series: "Curated Series",
    all_series: "All Series",
    view_archive: "View Archive",
    written_by: "Written by",
    reading_time: "min read",
    words: "words",
    next_page: "Next",
    prev_page: "Prev",
    back_to_home: "Back to Home",
    series: "Series",
    related_posts: "Related Articles",
    featured_articles: "Featured Articles",
    view_all: "View All",
    parts: "Parts",
    categories: "Categories",
    articles: "Articles",
    posts: "Articles",
    links: "Links",
    explore: "Explore",
    connect: "Connect",
    rss_feed: "RSS Feed",
    privacy: "Privacy",
    built_with: "Built with Amytis",
    on_this_page: "On this page",
    back_to_top: "Back to top",
    archive_subtitle: "{count} articles across {years} years.",
    archive_subtitle_one: "{count} articles across 1 year.",
    posts_subtitle: "{count} articles in total.",
    page_of_total: "Page {page} of {total}",
    series_subtitle: "{count} collections of curated knowledge.",
    series_subtitle_one: "1 collection of curated knowledge.",
    series_default_excerpt: "A growing collection of related thoughts.",
    shuffle_series: "Shuffle series",
    shuffle_posts: "Shuffle featured stories",
    shuffle_books: "Shuffle books",
    books_subtitle: "{count} long-form books and structured guides.",
    books_subtitle_one: "1 long-form book and structured guide.",
    tags_subtitle: "{count} topics spanning all articles and notes.",
    tags_subtitle_one: "1 topic spanning all articles and notes.",
    tag_posts_found: "{count} articles found.",
    tag_posts_found_one: "1 article found.",
    about_title: "About Amytis",
    about_subtitle: "Learn more about the philosophy and technology behind this digital garden.",
    view_all_posts: "View All {count} Articles",
    view_full_series: "View full series",
    collection: "Collection",
    view_full_collection: "View full collection",
    prev: "Prev",
    next: "Next",
    hide: "Hide",
    all_posts: "All Articles",
    books: "Books",
    book: "Book",
    chapter: "Chapter",
    chapters_count: "Chapters",
    start_reading: "Start Reading",
    all_books: "All Books",
    selected_books: "Selected Books",
    flow: "Flow",
    recent_notes: "Recent Notes",
    all_flows: "All Flows",
    no_posts: "No posts found.",
    no_flows: "No notes yet.",
    flow_subtitle: "{count} daily notes.",
    flows_in_year: "Notes in {year}",
    flows_in_month: "Notes in {month}",
    browse: "Browse",
    clear: "Clear",
    search_all: "All",
    recent_searches: "Recent",
    search_showing: "Showing {shown} of {total} results",
    search_unavailable: "Search is temporarily unavailable.",
    search_error: "Something went wrong with your search. Try again.",
    search_label: "Search",
    search_close: "Close search",
    search_filter_type: "Filter results by type",
    pagination: "Pagination",
    copy_code: "Copy code",
    code_copy: "Copy",
    code_copied: "Copied",
    code_wrap: "Wrap",
    code_no_wrap: "No wrap",
    word_wrap_enable: "Enable word wrap",
    word_wrap_disable: "Disable word wrap",
    prev_month: "Previous month",
    next_month: "Next month",
    expand_series: "Expand series",
    collapse_series: "Collapse series",
    series_navigation: "Series navigation",
    book_navigation: "Book navigation",
    clear_filter: "Clear filter",
    scrollable_content: "Scrollable content",
    scroll_left: "Scroll left",
    scroll_right: "Scroll right",
    toggle_theme: "Toggle theme",
    graph_loading: "Loading graph…",
    graph_unavailable: "Knowledge graph data is unavailable.",
    graph_search_placeholder: "Search nodes…",
    graph_aria_label: "Interactive knowledge graph of connected content",
    graph_nodes_count: "{count} nodes",
    nav_open_menu: "Open menu",
    nav_close_menu: "Close menu",
    nav_expand_submenu: "Expand {name}",
    nav_collapse_submenu: "Collapse {name}",
    not_found_title: "Page not found",
    not_found_message: "The page you are looking for does not exist or has been moved.",
    error_title: "Something went wrong",
    error_message: "An unexpected error occurred while rendering this page.",
    try_again: "Try again",
    search_results_found: "{total} results for \"{query}\"",
    search_no_results_for: "No results for \"{query}\"",
    search_type_post: "Article",
    search_type_flow: "Flow",
    search_type_book: "Book",
    search_tips: "Tips",
    search_tip_phrase: "Quoted string for exact matching (\" \" = Exact match)",
    search_tip_and: "Use spaces to combine keywords (Space = AND)",
    search_tip_exclude: "Exclude a term (- = Exclude)",
    discuss_post: "Discuss this post",
    share_post: "Share",
    copy_link: "Copy link",
    link_copied: "Copied!",
    flow_notes: "Flow Notes",
    tag_post_count: "{count} articles",
    tag_post_count_one: "1 article",
    tag_flow_count: "{count} flow notes",
    tag_flow_count_one: "1 flow note",
    tag_note_count: "{count} notes",
    tag_note_count_one: "1 note",
    tag_meta_description: "{count} entries tagged \"{tag}\".",
    subscribe: "Subscribe",
    subscribe_subtitle: "Stay updated with new articles and notes via your preferred channel.",
    rss_readers: "RSS Readers",
    rss_description: "Subscribe with any RSS reader for automatic updates when new content is published.",
    email_newsletter: "Email Newsletter",
    email_newsletter_description: "Get new articles delivered directly to your inbox.",
    telegram_channel: "Telegram",
    telegram_channel_description: "Instant updates via Telegram channel.",
    wechat_official: "WeChat Official Account",
    wechat_description: "Follow on WeChat for updates.",
    scan_qr_code: "Scan to follow",
    copy_feed_url: "Copy feed URL",
    feed_url_copied: "Copied!",
    join_channel: "Join Channel",
    subscribe_on_substack: "Subscribe on Substack",
    subscribe_via_email: "Subscribe via Email",
    social_connections: "Social",
    older: "Older",
    newer: "Newer",
    tab_all: "All",
    post_navigation: "Post navigation",
    filter_tags: "Filter tags",
    no_tags_found: "No tags found",
    more_tags: "+ {count} more",
    collapse_tags: "Show less",
    sort_popular: "Popular",
    sort_az: "A–Z",
    tags_count: "{shown} / {total} tags",
    tags_no_match: "No tags match \"{filter}\"",
    notes: "Notes",
    notes_subtitle: "{count} knowledge base notes.",
    graph: "Graph",
    backlinks: "Backlinks",
    graph_subtitle: "A visual map of connected knowledge.",
    search_type_note: "Note",
    all_notes: "All Notes",
    no_notes: "No notes yet.",
    more: "More",
    browser_outdated: "Your browser is outdated and may not display this site correctly.",
    browser_update: "Update your browser",
    browser_dismiss: "Dismiss",
    toc_expand: "Expand table of contents",
    toc_collapse: "Collapse table of contents",
    archive_description: "A complete chronological archive of all articles.",
    tags_description: "Explore topics spanning all articles and flow notes.",
    posts_description: "Browse all articles.",
    immersive_reading: "Immersive reading",
    exit_reading_mode: "Exit reading mode",
    reading_preferences: "Reading preferences",
    font_size: "Font size",
    reading_theme: "Theme",
    column_width: "Width",
    theme_auto: "Auto",
    theme_light: "Light",
    theme_sepia: "Sepia",
    theme_dark: "Dark",
    size_small: "S",
    size_medium: "M",
    size_large: "L",
    size_xl: "XL",
    width_narrow: "Narrow",
    width_medium: "Medium",
    width_wide: "Wide",
    width_full: "Full",
    collapse_sidebar: "Collapse sidebar",
    expand_sidebar: "Expand sidebar",
    reset_to_defaults: "Reset to defaults",
  },
  zh: {
    home: "首页",
    garden: "花园",
    archive: "归档",
    tags: "标签",
    about: "关于",
    search: "搜索...",
    search_placeholder: "搜索...",
    no_results: "未找到结果。",
    latest_writing: "最新文章",
    curated_series: "精选系列",
    all_series: "所有系列",
    view_archive: "查看归档",
    written_by: "作者",
    reading_time: "分钟阅读",
    words: "字",
    next_page: "下一页",
    prev_page: "上一页",
    back_to_home: "返回首页",
    series: "系列",
    related_posts: "相关文章",
    featured_articles: "精选文章",
    view_all: "查看全部",
    parts: "篇",
    categories: "分类",
    articles: "文章",
    posts: "文章",
    links: "链接",
    explore: "探索",
    connect: "连接",
    rss_feed: "RSS 订阅",
    privacy: "隐私政策",
    built_with: "基于 Amytis 构建",
    on_this_page: "本页目录",
    back_to_top: "返回顶部",
    archive_subtitle: "横跨 {years} 年，共 {count} 篇文章。",
    archive_subtitle_one: "共 {count} 篇文章，历时 1 年。",
    posts_subtitle: "共 {count} 篇文章。",
    page_of_total: "第 {page} 页，共 {total} 页",
    series_subtitle: "{count} 个精选知识合集。",
    series_subtitle_one: "1 个精选知识合集。",
    series_default_excerpt: "一个持续更新的文章合集。",
    shuffle_series: "随机排列系列",
    shuffle_posts: "随机排列精选文章",
    shuffle_books: "随机排列书籍",
    books_subtitle: "共 {count} 部长篇书籍与结构化指南。",
    books_subtitle_one: "1 部长篇书籍与结构化指南。",
    tags_subtitle: "共 {count} 个主题，横跨全部文章与随笔。",
    tags_subtitle_one: "1 个主题，横跨全部文章与随笔。",
    tag_posts_found: "找到 {count} 篇文章。",
    tag_posts_found_one: "找到 1 篇文章。",
    about_title: "关于 Amytis",
    about_subtitle: "了解这座数字花园背后的理念与技术。",
    view_all_posts: "查看全部 {count} 篇文章",
    view_full_series: "查看完整系列",
    collection: "合集",
    view_full_collection: "查看完整合集",
    prev: "上一篇",
    next: "下一篇",
    hide: "收起",
    all_posts: "全部文章",
    books: "书籍",
    book: "书籍",
    chapter: "章节",
    chapters_count: "章节",
    start_reading: "开始阅读",
    all_books: "所有书籍",
    selected_books: "精选书籍",
    flow: "随笔",
    recent_notes: "近期随笔",
    all_flows: "全部随笔",
    no_posts: "暂无文章。",
    no_flows: "暂无随笔。",
    flow_subtitle: "共 {count} 条随笔。",
    flows_in_year: "{year} 年随笔",
    flows_in_month: "{month} 随笔",
    browse: "浏览",
    clear: "清除",
    search_all: "全部",
    recent_searches: "最近",
    search_showing: "显示 {shown} / {total} 条结果",
    search_unavailable: "搜索暂不可用。",
    search_error: "搜索出错了，请重试。",
    search_label: "搜索",
    search_close: "关闭搜索",
    search_filter_type: "按类型筛选结果",
    pagination: "分页",
    copy_code: "复制代码",
    code_copy: "复制",
    code_copied: "已复制",
    code_wrap: "换行",
    code_no_wrap: "不换行",
    word_wrap_enable: "启用自动换行",
    word_wrap_disable: "禁用自动换行",
    prev_month: "上个月",
    next_month: "下个月",
    expand_series: "展开系列",
    collapse_series: "折叠系列",
    series_navigation: "系列导航",
    book_navigation: "书籍导航",
    clear_filter: "清除筛选",
    scrollable_content: "可滚动内容",
    scroll_left: "向左滚动",
    scroll_right: "向右滚动",
    toggle_theme: "切换主题",
    graph_loading: "正在加载图谱…",
    graph_unavailable: "知识图谱数据暂不可用。",
    graph_search_placeholder: "搜索节点…",
    graph_aria_label: "内容关联的交互式知识图谱",
    graph_nodes_count: "{count} 个节点",
    nav_open_menu: "打开菜单",
    nav_close_menu: "关闭菜单",
    nav_expand_submenu: "展开{name}",
    nav_collapse_submenu: "收起{name}",
    not_found_title: "页面不存在",
    not_found_message: "您访问的页面不存在或已被移动。",
    error_title: "出错了",
    error_message: "渲染此页面时发生了意外错误。",
    try_again: "重试",
    search_results_found: "找到 {total} 条\"{query}\"的结果",
    search_no_results_for: "未找到\"{query}\"的相关结果",
    search_type_post: "文章",
    search_type_flow: "随笔",
    search_type_book: "书籍",
    search_tips: "搜索技巧",
    search_tip_phrase: "加引号精确匹配短语（\" \" 表示精确匹配）",
    search_tip_and: "使用空格组合关键词（空格表示 AND）",
    search_tip_exclude: "排除关键词（- 表示排除）",
    discuss_post: "讨论这篇文章",
    share_post: "分享",
    copy_link: "复制链接",
    link_copied: "已复制",
    flow_notes: "随笔",
    tag_post_count: "{count} 篇文章",
    tag_post_count_one: "1 篇文章",
    tag_flow_count: "{count} 条随笔",
    tag_flow_count_one: "1 条随笔",
    tag_note_count: "{count} 条笔记",
    tag_note_count_one: "1 条笔记",
    tag_meta_description: "标记为「{tag}」的 {count} 条内容。",
    subscribe: "订阅",
    subscribe_subtitle: "通过您喜爱的方式订阅，及时获取新文章和随笔。",
    rss_readers: "RSS 阅读器",
    rss_description: "通过 RSS 阅读器订阅，发布新内容时自动获取更新。",
    email_newsletter: "邮件订阅",
    email_newsletter_description: "将新文章直接发送到您的邮箱。",
    telegram_channel: "Telegram 频道",
    telegram_channel_description: "通过 Telegram 频道获取即时更新。",
    wechat_official: "微信公众号",
    wechat_description: "关注微信公众号获取更新。",
    scan_qr_code: "扫码关注",
    copy_feed_url: "复制订阅链接",
    feed_url_copied: "已复制！",
    join_channel: "加入频道",
    subscribe_on_substack: "在 Substack 订阅",
    subscribe_via_email: "邮件订阅",
    social_connections: "社交媒体",
    older: "更早",
    newer: "更新",
    tab_all: "全部",
    post_navigation: "文章导航",
    filter_tags: "筛选标签",
    no_tags_found: "未找到标签",
    more_tags: "还有 {count} 个",
    collapse_tags: "收起",
    sort_popular: "热门",
    sort_az: "A–Z",
    tags_count: "{shown} / {total} 个标签",
    tags_no_match: "未找到匹配\"{filter}\"的标签",
    notes: "笔记",
    notes_subtitle: "共 {count} 条知识库笔记。",
    graph: "图谱",
    backlinks: "反向链接",
    graph_subtitle: "知识连接的可视化地图。",
    search_type_note: "笔记",
    all_notes: "全部笔记",
    no_notes: "暂无笔记。",
    more: "更多",
    browser_outdated: "您的浏览器版本过旧，可能无法正常显示本站内容。",
    browser_update: "更新浏览器",
    browser_dismiss: "关闭",
    toc_expand: "展开目录",
    toc_collapse: "收起目录",
    archive_description: "全部文章的时间轴归档。",
    tags_description: "浏览全部文章与随笔的主题标签。",
    posts_description: "浏览全部文章。",
    immersive_reading: "沉浸式阅读",
    exit_reading_mode: "退出阅读模式",
    reading_preferences: "阅读设置",
    font_size: "字号",
    reading_theme: "主题",
    column_width: "宽度",
    theme_auto: "自动",
    theme_light: "浅色",
    theme_sepia: "护眼",
    theme_dark: "深色",
    size_small: "小",
    size_medium: "中",
    size_large: "大",
    size_xl: "特大",
    width_narrow: "窄",
    width_medium: "中",
    width_wide: "宽",
    width_full: "全宽",
    collapse_sidebar: "收起目录",
    expand_sidebar: "展开目录",
    reset_to_defaults: "恢复默认",
  },
};

export type Language = keyof typeof translations;
export type TranslationKey = keyof typeof translations.en;
